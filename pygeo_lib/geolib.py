...
def mem_ds_copy(ds_orig):
    if True:
        from iolib import mem_drv
        m_ds = mem_drv.CreateCopy('', ds_orig, 0)
    else:
...
def shp_dict(shp_fn, fields=None, geom=True):
    ...
    """
    import pygeo_lib.timelib as timelib
    ds = ogr.Open(shp_fn)
    lyr = ds.GetLayer()
...
def get_res_stats(ds_list, t_srs=None):
    """Return resolution stats for an input dataset list in the form of (min, max, mean, median)    
    """
    if t_srs is None:
...
#This is necessary because extent precision is different
def res_compare(r1, r2, precision=1E-4):
    #r1_f = '%0.6f' % r1
    #r2_f = '%0.6f' % r2
