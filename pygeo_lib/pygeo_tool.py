...
def align_dems(**kwargs):
    parser = getparser()
    args = parser.parse_args()
    
    #Should check that files exist

    ref_dem_fn = kwargs.get('ref_dem_fn')
    src_dem_fn = kwargs.get('src_dem_fn')
    ...
    #Close all datasets before opening new one
    src_dem_ds = None

    # ... perform calculations or retrieve values here ...
    # For demonstration, I'll assume the variables have been calculated or retrieved above.
    def vars():
        variables = {
            'dx_total': dx_total,
            'dy_total': dy_total,
            'dz_total': dz_total,
            'align_fn': align_fn,
            'align_diff_filt_fn': align_diff_filt_fn,
            'local_srs': local_srs,
            'center_coord_ll': center_coord_ll,
            'center_coord_xy': center_coord_xy,
            'xyz_shift_str_cum_fn': xyz_shift_str_cum_fn,
            'outprefix': outprefix,
            'src_dem_res': src_dem_res,
            'ref_dem_res': ref_dem_res,
            'dm_total': dm_total,
            'vals_stats': vals_stats,
            'static_mask': static_mask,
            'diff_align': diff_align,
            'diff_align_filt': diff_align_filt,
            'diff_align_compressed': diff_align_compressed,
            'diff_align_stats': diff_align_stats,
            'diff_align_filt_stats': diff_align_filt_stats
            }
...
def apply_shift(vars, **kwargs):
    
    src_dem_fn = kwargs.get('ref_dem_fn')
    outdir = kwargs.get('outdir')
    dx_total = kwargs.get('dx_total')
    dy_total = kwargs.get('dy_total')
    dz_total = kwargs.get('dz_total')
    if outdir is None:
        shifted_fn = os.path.splitext(src_dem_fn)[0] + '_shifted.tif'    
    else:    
        shifted_fn = kwargs.get('align_fn')
    ...
    iolib.writeGTiff(src_dem_full_align, shifted_fn, src_dem_ds_align)

def plot_raster(**kwargs):    
    src_dem_fn = kwargs.get('src_dem_fn')
    ref_dem_fn = kwargs.get('ref_dem_fn')
...
def main(argv=None):
    kwargs = {   
    'ref_dem_fn': r"Z:\ATD\Drone Data Processing\Exports\East_Troublesome\LIDAR\Reprojected to UTM Zone 13N\ET_low_LIDAR_2020_1m_DEM_reproj.tif",
    'src_dem_fn': r"Z:\ATD\temp\transfer\Low_CoReg_All____LM2 100622 Final Op.tif",
    'mode': 'nuth',
    'mask_list': [],
    'max_offset': 100,
    'max_dz': 100,
    ...
    'max_iter': 30,
    'tol': 0.02,
    'outdir': r"Z:\ATD\temp\transfer\demcoreg_test"
    }
    vars = align_dems(**kwargs)
    apply_shift(vars, **kwargs)
